...
const Nav = () => {
  ...
  const handleClick = (e) => {
    ...
    router.push('/').then(() => {
      router.reload(); // Force a reload after redirecting
    });
  };
  ...
  return (
    <div className='flex justify-between items-center h-[80px] border-b-[1px] border-gray-400'>
      <div className='w-[80px]  font-bold text-[25px]'>

        <Link href={'/'} onClick={handleClick}>
          <Image src="/logo-black.png" width={100} height={70} style={{ objectFit: "cover", height: "100%", width: "100%" }} />
        </Link>
      ...
      {status === "authenticated" ? (
        <div>
          <div className='hidden lg:flex text-gray-900 font-bold gap-5 items-center'>
            ...
            {data?.user?.image && (
              <div className='rounded-full overflow-hidden h-10'>
                <img style={{ objectFit: 'cover', width: "100%", height: "100%" }} src={data.user.image} alt="" />
          ...
          <div className='lg:hidden flex justify-center items-center gap-5 relative'>
            ...
            <div onClick={toggleMenu} className={`absolute bg-white rounded-md border-2 top-12 right-0 z-20 w-[180px] flex flex-col gap-3 p-4 text-blue-500 ${menuVisible ? '' : 'hidden'}`}>
              <Link href={`/${data.user.email}`} className=' text-blue-500'>Your Blogs</Link>
        ...
        <div>
          <Link href={'/login'} className='flex justify-center text-blue-500 items-center gap-2 border-2 border-blue-500 px-4 py-3 rounded-full'>Login</Link>
        </div>
      )}
